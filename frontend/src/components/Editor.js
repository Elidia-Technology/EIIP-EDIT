...
export default function Editor() {
  ...
  // Mouse event handlers for interactive crop
  ...
  const handleCanvasMouseDown = (e) => {
    if (!cropMode || !currentImageURL) return;
  ...
  const handleCanvasMouseMove = (e) => {
    if (!cropMode || !isDragging || !cropStart) return;
    const coords = getCanvasCoordinates(e);
    setCropEnd(coords);
  ...
  React.useEffect(() => {
    if (selectedTool === 'crop') {
      ...
      if (currentImageURL) {
        displayImageOnCanvas(currentImageURL);
      }
    }
  }, [selectedTool, currentImageURL]);
  ...
  const handleFlip = async (direction) => {
    ...
    if (!currentImageURL || cropWidth <= 0 || cropHeight <= 0) return;
    setProcessing(true);
    try {
      ...
      console.log(`✅ Cropped: ${cropWidth}x${cropHeight} at (${cropX}, ${cropY})`);
    } catch (error) {
  ...
  const handleSharpen = async () => {
    ...
    setProcessing(true);
    try {
      const result = await eiip.applyEffect(currentImageURL, 'sharpen', sharpenAmount, {
        quality: 0.9,
        format: 'png'
      });
      ...
      alert('Sharpen failed: ' + error.message);
    } finally {
      setProcessing(false);
    }
  };

  const handleVintage = async () => {
    if (!currentImageURL) return;
    setProcessing(true);
    try {
      const result = await eiip.applyEffect(currentImageURL, 'vintage', 1.0, {
        quality: 0.9,
        format: 'png'
      });
      ...
      alert('Vintage effect failed: ' + error.message);
    } finally {
      setProcessing(false);
    }
  };
  ...
  const handleWarm = async () => {
    ...
    setProcessing(true);
    try {
      const result = await eiip.applyEffect(currentImageURL, 'warm', 1.0, {
        quality: 0.9,
        format: 'png'
      });
      ...
      setCurrentImageURL(result.dataUrl);
      displayImageOnCanvas(result.dataUrl);
      console.log('✅ Warm effect applied');
    } catch (error) {
      console.error('❌ Warm error:', error);
      alert('Warm effect failed: ' + error.message);
    } finally {
      setProcessing(false);
    }
  };
  ...
  const handleCool = async () => {
    ...
    setProcessing(true);
    try {
      const result = await eiip.applyEffect(currentImageURL, 'cool', 1.0, {
        quality: 0.9,
        format: 'png'
      });
      ...
      setCurrentImageURL(result.dataUrl);
      displayImageOnCanvas(result.dataUrl);
      console.log('✅ Cool effect applied');
    } catch (error) {
      console.error('❌ Cool error:', error);
      alert('Cool effect failed: ' + error.message);
    } finally {
      setProcessing(false);
    }
  };
  ...
  const handleScale = async () => {
    ...
    setProcessing(true);
    try {
      const result = await eiip.scaleImage(currentImageURL, scaleFactor, {
        quality: 0.9,
        format: 'png'
      });
      ...
      setCurrentImageURL(result.dataUrl);
      displayImageOnCanvas(result.dataUrl);
      console.log(`✅ Scaled: ${scaleFactor}x`);
    } catch (error) {
  ...
  const handleAddText = async () => {
    ...
    setProcessing(true);
    try {
      const result = await eiip.addText(currentImageURL, {
        text: textContent,
      ...
      setCurrentImageURL(result.dataUrl);
      displayImageOnCanvas(result.dataUrl);
      console.log(`✅ Text added: ${textContent}`);
    } catch (error) {
      console.error('❌ Add text error:', error);
      alert('Add text failed: ' + error.message);
    } finally {
      setProcessing(false);
    }
  };
  ...
  const handleOptimize = async () => {
    ...
    setProcessing(true);
    try {
      const result = await eiip.optimizeImage(currentImageURL, {
        maxWidth: 1920,
        maxHeight: 1920,
        quality: 0.85,
        format: 'jpeg'
  ...
  const handleThumbnail = async () => {
    ...
    setProcessing(true);
    try {
      const result = await eiip.createThumbnail(currentImageURL, {
        width: 200,
        height: 200,
        fit: 'cover',
        quality: 0.9,
      ...
      setCurrentImageURL(result.dataUrl);
      displayImageOnCanvas(result.dataUrl);
      console.log('✅ Thumbnail created: 200x200');
    } catch (error) {
      console.error('❌ Thumbnail error:', error);
      alert('Thumbnail creation failed: ' + error.message);
    } finally {
      setProcessing(false);
    }
  };
  ...
  const renderToolPanel = () => {
    ...
    switch (selectedTool) {
      ...
      case 'crop':
        return (
          <div className={panelClass}>
            ...
            <div className="space-y-4">
              <div className="bg-[#2a2a2a] border border-[#0078d4] rounded p-3 mb-4">
                <p className="text-xs text-gray-300 leading-relaxed">
                  <span className="font-semibold text-[#0078d4]">💡 Tip:</span> Click and drag on the image to select the crop area. The selection will appear with blue handles.
                </p>
              </div>
              <div className={sectionClass}>
              ...
              <button onClick={handleCrop} className={btnClass} disabled={processing || cropWidth <= 0 || cropHeight <= 0}>
          ...
          <div className={panelClass}>
            ...
            <div className="space-y-2">
              <div className="grid grid-cols-2 gap-2">
                <button onClick={handleGrayscale} className={btnSecondaryClass} disabled={processing}>
                  Grayscale
                </button>
                ...
                <button onClick={handleVintage} className={btnSecondaryClass} disabled={processing}>
                  Vintage
                </button>
                <button onClick={handleWarm} className={btnSecondaryClass} disabled={processing}>
                  Warm
                </button>
                <button onClick={handleCool} className={btnSecondaryClass} disabled={processing}>
                  Cool
                </button>
              </div>
              <div className={sectionClass}>
                <label className={labelClass}>Sharpen Amount: {sharpenAmount}</label>
                <input type="range" value={sharpenAmount} onChange={(e) => setSharpenAmount(Number(e.target.value))} min="0" max="2" step="0.1" className="w-full h-2 bg-[#2a2a2a] rounded-lg appearance-none cursor-pointer accent-[#0078d4]" />
              </div>
              <button onClick={handleSharpen} className={btnClass} disabled={processing}>
          ...
          <div className={panelClass}>
            ...
            <div className="space-y-4">
              <div className={sectionClass}>
                ...
                <input type="text" value={watermarkText} onChange={(e) => setWatermarkText(e.target.value)} className={inputClass} placeholder="Watermark..." />
              </div>
      ...
      case 'text':
        return (
          <div className={panelClass}>
            <h3 className="text-sm font-semibold mb-4 text-gray-200 uppercase tracking-wide border-b border-[#4a4a4a] pb-2">Add Text</h3>
            <div className="space-y-4">
              <div className={sectionClass}>
                <label className={labelClass}>Text Content</label>
                <input type="text" value={textContent} onChange={(e) => setTextContent(e.target.value)} className={inputClass} placeholder="Enter text..." />
              </div>
              <div className={sectionClass}>
                <label className={labelClass}>Position</label>
                <select value={watermarkPosition} onChange={(e) => setWatermarkPosition(e.target.value)} className={inputClass}>
              ...
              <div className={sectionClass}>
                <label className={labelClass}>Font Size: {watermarkFontSize}px</label>
                <input type="range" value={watermarkFontSize} onChange={(e) => setWatermarkFontSize(Number(e.target.value))} min="12" max="120" className="w-full h-2 bg-[#2a2a2a] rounded-lg appearance-none cursor-pointer accent-[#0078d4]" />
              </div>
              <div className={sectionClass}>
              ...
              <button onClick={handleAddText} className={btnClass} disabled={processing}>
      ...
      case 'scale':
        return (
          <div className={panelClass}>
            <h3 className="text-sm font-semibold mb-4 text-gray-200 uppercase tracking-wide border-b border-[#4a4a4a] pb-2">Scale Image</h3>
            <div className="space-y-4">
              <div className={sectionClass}>
                <label className={labelClass}>Scale Factor: {scaleFactor}x</label>
                <input type="range" value={scaleFactor} onChange={(e) => setScaleFactor(Number(e.target.value))} min="0.1" max="3" step="0.1" className="w-full h-2 bg-[#2a2a2a] rounded-lg appearance-none cursor-pointer accent-[#0078d4]" />
              </div>
              <div className="grid grid-cols-3 gap-2">
                <button onClick={() => { setScaleFactor(0.5); }} className={btnSecondaryClass}>0.5x</button>
                <button onClick={() => { setScaleFactor(1.0); }} className={btnSecondaryClass}>1.0x</button>
                <button onClick={() => { setScaleFactor(2.0); }} className={btnSecondaryClass}>2.0x</button>
              </div>
              <button onClick={handleScale} className={btnClass} disabled={processing}>
                {processing ? 'Processing...' : 'Apply Scale'}
              </button>
            </div>
          </div>
        );
      ...
      case 'optimize':
        return (
          <div className={panelClass}>
            ...
            <div className="space-y-3">
              <button onClick={handleOptimize} className={btnClass} disabled={processing}>
                {processing ? 'Processing...' : 'Auto Optimize (1920px)'}
              </button>
              <button onClick={handleThumbnail} className={btnSecondaryClass} disabled={processing}>
                {processing ? 'Processing...' : 'Create Thumbnail (200px)'}
              </button>
            </div>
          </div>
        );
        ...
        {sidebarOpen && (
          <aside className={`${sidebarBgClass} w-16 border-r border-[#1a1a1a] panel-border flex flex-col items-center py-3 space-y-1 overflow-y-auto`}>
            {[
              ...
              { id: 'scale', icon: '⚖️', label: 'Scale', shortcut: 'X' },
              ...
              { id: 'watermark', icon: '©️', label: 'Watermark', shortcut: 'W' },
              { id: 'optimize', icon: '⚡', label: 'Optimize', shortcut: 'O' },
